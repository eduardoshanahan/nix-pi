...
- Build a custom **NixOS SD card image** for Raspberry Pi (AArch64) with:
  - DHCP enabled
  - SSH enabled (key-only)
  - Your SSH public key baked into `authorized_keys`
- Flash SD card
- Boot Pi headless (Ethernet)

Build targets live in this repo’s root flake:
...
Ubuntu/x86_64 + QEMU build notes are in `documentation/BUILDING_SD_IMAGE_ON_UBUNTU_X86.md:1`.
