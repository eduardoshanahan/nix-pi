...
- Assign each Pi a unique hostname: `pi-node-a`, `pi-node-b`, `pi-node-c`, …
...
- Enable mDNS/Avahi (already enabled in `modules/networking.nix`) so `pi-node-a.local` works on many networks.
...
- `./scripts/pull-hardware pi-node-a root@<pi-ip>`

This copies `/etc/nixos/hardware-configuration.nix` from the Pi into `hosts/pi-node-a/hardware-configuration.nix`.

## 4) Deployment workflow (normal operations)

Use `nixos-rebuild` from the workstation:

- `./scripts/deploy pi-node-a root@<pi-ip>`
...
- `BUILD_HOST=<ssh-host> ./scripts/deploy pi-node-a root@<pi-ip>`

To build on the workstation (if it can build aarch64), use:

- `BUILD_HOST=local ./scripts/deploy pi-node-a root@<pi-ip>`
...
1. Create `hosts/rpi-box-NN/default.nix` (copy from `hosts/pi-node-a/default.nix`)
